...
export function useDragAndDrop(
  ...
  onReorder: (draggedTaskId: string, targetTaskId: string, status: TaskStatus) => void,
  columnStatus: TaskStatus // Add as parameter to the hook instead
) {
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    // If dragging over a task card, highlight the drop target
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    // Find the target task if dropped on a task card
    const taskCard = (e.target as HTMLElement).closest('.task-card') as HTMLElement;
    const targetTaskId = taskCard?.getAttribute('data-task-id') || null;
    
    // Case 1: Reordering within the same column
    if (sourceStatus === columnStatus && targetTaskId && targetTaskId !== taskId) {
      onReorder(taskId, targetTaskId, columnStatus);
      return;
    }
    
    // Case 2: Status change between columns
